...
once it is resumed on return from a system call or kernel interrupt. A
variant of this function, `ccx_createContextShareStack()`, supports stack
sharing beteen two or more contexts.
...
object as the suspended start and the same or similar preparatory
functions are used to create this object. Effectively, the suspended
context creation for later resume is no longer an essential element --
however still useful for typical constructs.]
...
implementation each.footnote:[
  With the exception of `common`, which common code of all or some of the
samples.]
