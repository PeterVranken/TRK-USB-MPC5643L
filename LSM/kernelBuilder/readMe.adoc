...
There is actually one more path through the IVOR handlers: A system call
can be a so called simple one, which is a kernel unrelated system call. It
can not command a context switch and will always return to the calling
context. From the behavior such a simple system call is like an ordinary
function call, it takes arguments and returns a result. The major
difference to an ordinary function is its execution in supervisor mode.
...
their APIs. The current centralized table requires cumbersome sharing of
various handlers. Proper separation of driver implementation and kernel
is complicated.]
...
`printf` and only used by the client code. Package `serial` was extended
by a wrapper around the driver API so that it is available to contexts
running in user mode. The wrapper implements the same API as system calls.
