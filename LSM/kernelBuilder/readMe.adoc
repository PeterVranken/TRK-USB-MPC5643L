...
single core operating system kernel for the NXP MPC5643L (e200z4). The IVOR #4
...
implemented in sheer C code. A complete, cooperative or preemptive
...
ISRs can no longer be of type `void (*)(void)` -- they require a
signature, which permits commanding the context switch on return.
...
system call, which wraps the generic call `init_systemCall(idxSysCall, ...)`
with a meaningful name.]footnote:[
...
design in order to properly separate the implementation of the kernel and
different drivers. The samples demonstrate how this can be done.]
...
switches). Only start and termination make a difference. See next section.
...
* `ccx_createContext()` expects the same arguments as the first function.
  ...
  the C entry function. The context doesn't need to be started an more
...
reached then the context will be started or resumed, which implements the
task.

The concept of kernelBuilder is that handlers for these events, which are
implemented client code, do all the organizational work, which is required
to keep track of which context should be served next and on return they
will tell kernelBuilder's underlying assembler code what to do.
...
left context and in order to say where to store the information about
...
maintenance of pointer to active task, etc., happens necessarily before
(inside the handler) the taken decision, the yielded context switch can be
performed (after return from the handler). This is no issue because of the
race condition free implementation paradigm for the client scheduler code.
All kernel relevant handler invocations, External Interrupts and system
calls, are serialized. A handler will never be preempted by another one
and even less by a context under control of the scheduler. kernelBuilder
applies the priority ceiling protocol for doing so, which means that
kernel unrelated External Interrupts can still preempt all the scheduler
code. (Therefore they must not make use of scheduler functions without
additional, explicit synchronization code.)

A handler, which requests a context switch on return can furthermore
specify a result value for the entered context; if this context is started
...
value. Leaving the entry function is a totally different thing than
commanding context termination at return from a kernel relevant IVOR
...
`int_fctOnContextEnd()`. It's argument is the value returned from the
entry function. This function is executed still in the same context as the
left entry function and executing `int_fctOnContextEnd()` needs to the
virtually last thing a context can do. However, this function is not an
IVOR handler, it is not executed in the scheduler context, it can not
command context termination on return. Instead, the client code will
likely put a system call in the body of this function will has the meaning
"signal end of task". The system call implementation -- an IVOR handler --
will update the scheduler data structure to reflect the changed task state
and command the task termination on return in order to do the stack
cleanup.
...
active task, which is the one. Whichever that will be -- but the scheduler
knows of course.

=== Task sharing

TODOC: Stack sharing. Conditions, when sharing is possible, typical use
case BCC tasks
...
can be resumed by the scheduler on whatever events.
+
...
+
Choosing scenario 2. and 3. doesn't make a significant difference in
...
contexts, which terminate by leaving the context entry function.
...
=== TODOC: The next paragraphs require another section title, rearrange text
