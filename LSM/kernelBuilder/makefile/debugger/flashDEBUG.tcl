...
if {[info exists APP]  &&  $APP!=""  &&  [info exists INSTR]  &&  ($INSTR=="BOOK_E" || $INSTR=="VLE")} {
    ...
    if {$INSTR=="BOOK_E"} {
        fl::target -lc "kernelBuilder ($APP, Book E, DEBUG)"
    } else {
        fl::target -lc "kernelBuilder ($APP, VLE, DEBUG)"
    }
    ...
    if {$INSTR=="BOOK_E"} {
        cmdwin::fl::image -f ..\\..\\bin\\ppc-BookE\\$APP\\DEBUG\\TRK-USB-MPC5643L-kernelBuilder.elf -t "Auto Detect" -re on -r 0x0 0xfffff -oe off
    } else {
        cmdwin::fl::image -f ..\\..\\bin\\ppc-VLE\\$APP\\DEBUG\\TRK-USB-MPC5643L-kernelBuilder.elf -t "Auto Detect" -re on -r 0x0 0xfffff -oe off
    }
    ...
    puts "kernelBuilder application $APP has been flashed in DEBUG compilation in $INSTR implementation"
