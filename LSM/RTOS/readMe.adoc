= Sample "RTOS" - A simple but effective RTOS for MPC5643L
...
any further task occupies a interrupt capable (timer) device.
...
section outline, which functions are available and how to use them.
...
ceiling protocol. The currently running task is given a new, higher
priority and all other tasks of same or lower priority will surely not
become running. There's a function to raise and another one to restore the
original priority:
