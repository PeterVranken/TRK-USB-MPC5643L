...
nothing about ADC. If an application wants to use analogue inputs then
it'll add the hardware configuration of the ADC as appropriate. The two
mentioned files contain only the basic initialization and most of this
...
handler has been implemented for you footnote:[It could be exchanged with
your own handler like it has been described for any trap handler in
section <<How to install your own application trap handlers>>].
