...
/** On-DMA-Complete interrupt and API function sio_OS_writeSerial() need to share the
...
#define IRQ_MASK    (0x1 << DMA_CHN_FOR_SERIAL_OUTPUT)
    ...
    else
    {
        /* No subsequent DMA transfer is immediately initiated, so the application code
           will need to start one the next time the API function sio_OS_writeSerial is
           called. */
                                     ...
                                     )
{
    ...
    extern uint8_t ld_romStart[0], ld_romEnd[0]
                 , ld_ramStart[0], ld_ramEnd[0];
    const uintptr_t adrMsg = (uintptr_t)msg
                  , adrMsgEnd = adrMsg + noBytes
                  , adrRomStart = (uintptr_t)ld_romStart
                  , adrRomEnd = (uintptr_t)ld_romEnd
                  , adrRamStart = (uintptr_t)ld_ramStart
                  , adrRamEnd = (uintptr_t)ld_ramEnd;
    if(!(adrMsg >= adrRomStart  &&  adrMsgEnd <= adrRomEnd
         ||  adrMsg >= adrRamStart  &&  adrMsgEnd <= adrRamEnd
        )
      )
    {
