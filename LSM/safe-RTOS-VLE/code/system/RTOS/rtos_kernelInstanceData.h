...
typedef struct rtos_kernelInstanceData_t
{
    ...
    /** A pointer to event, which has to be served next by the scheduler. Points to an
        element of array rtos_getInstancePtr()->eventAry if an event requires the call of
 ...
 *   @remark
 * This function my be called from all contexts, OS and user mode tasks and ISRs.
 */
static inline rtos_kernelInstanceData_t *rtos_getInstancePtr(void)
{
