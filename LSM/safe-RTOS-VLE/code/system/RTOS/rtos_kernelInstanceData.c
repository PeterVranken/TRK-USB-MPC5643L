...
/** Global data of one instance of the RTOS on a multi-core MCU. */
rtos_kernelInstanceData_t DATA_OS(rtos_kernelInstanceDataAry)[RTOS_NO_CORES] =
{
    /* All fields have zero or "invalid" initial values. We can use the same initializer
       expression for all CPU cores */
    [0 ... (sizeOfAry(rtos_kernelInstanceDataAry)-1)] =
    {
        ...
        /* A pointer to the event, which has to be served next by the scheduler. Points to
           an element of array rtos_getInstancePtr()->eventAry if an event requires the
