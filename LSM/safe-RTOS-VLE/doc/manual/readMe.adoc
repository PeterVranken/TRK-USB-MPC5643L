...
control, is depicted in the left part of <<figMemMap>>. With respect to
flash ROM and address space of the peripherals, this part of the image is
already detailed enough; the used flash ROM and the entire peripheral
space are memory chunks, which are not further divided and which are
controlled with one memory area descriptor in the MPU each.
...
object to be in a _data_ or _bss_ section, which would normally go into a
...
process _P1_, too. All memory, which is got from the C lib's _malloc_
