...
only when we return to the stack frame of the preempted, deadline
monitored task: Now the end time is easily accessible and killing the task
in case is simply doable, because it's on top of the stack.
