...
* Configuration and initialization functions are generally restricted to
  OS use only and are not tagged. They use the pattern for application
  run-time functions. (A decision, which must be doubted and could lead
  to a change of the API in the future)
...
for the price of an a bit higher execution time.
