 ...
 ** Not critical: If there's no IRQ before it only matters to tasks of
    lower priority, which must not be safety critical by design
* An exception leaves task's data in inconsistent state. (Can abort task
  in the middle of a critical section.) Generally no design tool for
  limiting execution time. Just an error recognition and worst case
  hindering tool
...
to preemptable ISRs, OS and to user tasks and system call handlers of full
conformance class. Non-preemptable ISRs and system call handlers of lower
conformance class must not use it.
...
to demonstrate the use of the APIs to configure an run the RTOS: Create an
