...
with its static use of the MPU. The complete address space, we have to
control is depicted in <<figMemMap>>. With respect to flash ROM and
address space of the peripherals, the shown map is already detailed
enough; the used flash ROM and the entire peripheral space are memory
chunks, which are not further divided and which are controlled with one
memory area descriptor in the MPU each.

[[figMemMap]]
.Memory Map of MPC5643L
image::readMe_resources/memoryMap.svg[Memory Map of MPC5643L, pdfwidth=50%, width=50%, align="center"]
...
descriptors having up to four user processes (the remaining descriptors are
required for kernel RAM, ROM, peripherals and shared RAM).

//<<figMemMapRAM>> details how the used RAM is further divided into process related memory chunks.
...
The filters route the input section to the process memory areas. So if we
...
double-check the locating of data objects.
...
Normally, the type decoration is required only for the object definition,
but rarely you will need to place the same at a publicly visible
declaration in the header file, too. This will happen if you force a data
object to be in a data or bss section, which would normally go into a
small data area. "Normally" means decided by the compiler's internal rules.
Data objects with a size of up than 8 Byte would for example be normally
placed in the small data area. If the declaration of such a data object
doesn't contain the section attribute and when compiling another source
file, which only reads the declaration, then the compiler will emit code
with short addressing mode while the variable is not in a small data area.
The linker will refuse to resolve the address offsets in the
short-addressing-{zwsp}mode-instructions in the object file of that other
source file.

Note, there's no support for shared objects in small data areas.
...
wrong or missing type decoration for a data objects. A variable without
...
accessible by all calling processes. Maybe, the solution is the use of the
shared memory area but this can easily break the safety concept.
...
intended to host the functional code, which normally is the majority of
code. This code can still be used with the RTOS without the need to "grep"
for all data definitions in order to add the type decoration.
...
. Look for all data definitions and add the macros to make the objects be
  owned by process _P2_.
...
The C library places all its static data objects in the normal data and
bss sections. Its source code does of course not make use of our macros
and all static data objects are owned by process _P1_. Therefore, _P1_
is the only process, which can make safe use of the C lib functions.

Many, if not most functions from the library won't make use of static
data and could therefore be used from other processes, too. Regardless, this
