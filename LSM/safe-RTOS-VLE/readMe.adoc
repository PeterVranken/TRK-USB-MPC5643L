...
new features of the RTOS. Several tasks are running concurrently and the
...
Some more meaningful application of the RTOS can be found in folder
`code\application\`.
...
* There's no longer a limitation for the maximum number of events (and,
  ...
  change; a port for the MPC5775B/E dual-core suffered from the hardware
  limitation of eight events, which were even to be distributed onto both
  cores
...
. A similar effect has been observed with the instructions to alter the
External Interrupt enable bit, MSR[EE]. Do not single-step in the debugger
over wrtee(i) instructions. The instruction may fail to change the bit. If
the code approaches such an instruction you should use the right-click
operation "Run to line", targeting the instruction behind the wrtee(i).
This works fine.
