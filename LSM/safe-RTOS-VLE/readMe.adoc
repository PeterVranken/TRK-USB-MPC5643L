...
- Secured priority ceiling protocol for communication
...
user code (if permission is granted) or from ISRs belonging to the
operating system.

The RTOS offers a pre-configured set of up to four processes. (The
limitation to four is a hardware constraint and no virtualization by
software has been shaped for sake of simplicity.) The operating system
forms a fifth process. Tasks need to registered. The registration assigns
them to one of the processes and associates them with one of the created
events.

All scheduling is strictly priority controlled. The triggering of an event
makes all associated tasks ready to run. At any time, the scheduler decides by
priority, which of the _ready_ tasks becomes the one and only _running_
task. This may involve preemption of tasks.

ISRs can be installed similar to the other samples; this is done with the
...
It is secured so that safety tasks cannot be accidentally or purposely
blocked.

The already known mechanisms from the startup code to suspend and resume
all interrupts are still supported but they are no longer offered to
application code, only the operating system may use them (i.e. mainly I/O
...
details can be found in the manual of the RTOS (TBD).
...
self-designed GUI, there need to the self-made system control software,
...
unpowered. That's all. A few hundred lines of code will typically suffice
for such a task.
...
- If the implementation of a task, which is meant the supervisory or
  ...
  imaginable failures are made by any other task.
...
RAM areas. (And to one additional shared area, which all process have write
access to.) Writing to any other address of the address space is a
privilege violation.
...
Reading MCU registers (CPU supervisor registers and I/O registers) can
have side-effects and is therefore generally forbidden. Any attempt to do
so is considered a privilege violation.
...
It'll not take any decision like: "Enough is enough, we are going to shut
down." However, it offers the mechanisms to implement such decisions in a
particular task.
...
Note, the execution of code in ROM is basically not restricted. A task may
...
be the case - except for implementation errors in the kernel (or in an
added I/O driver, see below) or, less likely but possible, a sporadic
hardware fault, e.g. caused by local chip overtemperature or cosmic
radiation. In which case the kernel simply stops working. Halting the
software execution can be considered not breaking the safety concept of
the system; there will be an external (i.e. CPU unrelated) device which
permanently checks the alive status of the software running on the CPU and
this device will ensure the transition of the system into the safe state.
...
and tasks and I/O device drivers.
...
terms of CPU load and ease of programming. The by far leanest "basic"
implementation needs to be done in assembler, the other two classes can be
implemented in C.

Note, adding an I/O driver to the RTOS is not only a matter of obeying the
rules from the driver model. Any kind of programming error can break the
