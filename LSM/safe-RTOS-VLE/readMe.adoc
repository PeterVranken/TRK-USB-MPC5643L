...
which were the simple RTOS without safety concept and safe-RTOS-VLE from
the master branch. This simplicity significantly supports the validation
of the code in a safety-critical software development environment.

The major difference between safe-RTOS on this branch and on the master
branch is the substitution of the hardware based scheduler by a pure
software solution. This has its pros and cons:
...
* There's no longer a limitation for the maximum number of events (and,
  ...
  change; a port for the MPC5775B/E dual-core suffered from the eight
  events, which were even to be distributed onto both cores
