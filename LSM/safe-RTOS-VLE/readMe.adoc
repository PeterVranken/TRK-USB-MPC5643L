...
TRK-USB-MPC5643L sample RTOS-VLE has been modified as
`code\application\default\mai_main.c` in order to make use of some of the
new features of the RTOS. The functionality is similar to sample "startup"
...
The architecture of the RTOS is very simple and resemble its ancestor.
This simplicity significantly supports the validation of the code in a
safety-critical software development environment. The major difference
between safe-RTOS and its ancestor is the substitution of the hardware
...
* The performance of the software solution is worse. The sample
  application "default" shows an increased CPU load by about 1.5%
* There's no longer a limitation for the maximum number of events (and,
  effectively, the number of tasks). This was the main motivation for the
  change; a port to the MPC5775B/E dual-core suffered from the eight
  events, which were to be distributed on both cores
* In a safety aware development process, the effort for testing and code
  reviews will be significantly higher for the software solution
...
documentation of the ancestor RTOS, please look
https://github.com/PeterVranken/TRK-USB-MPC5643L/tree/master/LSM/RTOS-VLE#The-RTOS[here].

The differences between safe-RTOS with software scheduler and the simple
RTOS are:

* The safety concept
* Any number of events can be created and all software interrupts remain
...
* All tasks have a prioprity, which is by principle lower than any
  interrupt priority
* User mode tasks can't have a critical section with ISRs. (OS tasks still
  can have)
* The architecture may be similar but the implementation isn't. The safety
  demands require a significant portion of assembler code for
  implementation
