...
if {[info exists APP]  &&  $APP!=""  &&  [info exists INSTR]  &&  ($INSTR=="BOOK_E" || $INSTR=="VLE")} {
    ...
    if {$INSTR=="BOOK_E"} {
        fl::target -lc "RTuinOS ($APP, Book E, PRODUCTION)"
    } else {
        fl::target -lc "RTuinOS ($APP, VLE, PRODUCTION)"
    }
    ...
    if {$INSTR=="BOOK_E"} {
        cmdwin::fl::image -f ..\\..\\bin\\ppc-BookE\\$APP\\PRODUCTION\\TRK-USB-MPC5643L-RTuinOS-$APP.elf -t "Auto Detect" -re on -r 0x0 0xfffff -oe off
    } else {
        cmdwin::fl::image -f ..\\..\\bin\\ppc-VLE\\$APP\\PRODUCTION\\TRK-USB-MPC5643L-RTuinOS-$APP.elf -t "Auto Detect" -re on -r 0x0 0xfffff -oe off
    }
    ...
    puts "RTuinOS has been flashed with application $APP in PRODUCTION compilation in $INSTR implementation"
