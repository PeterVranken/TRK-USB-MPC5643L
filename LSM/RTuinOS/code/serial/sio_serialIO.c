...
/** Sometimes for testing, it's annoying to have additional interrupts and asynchronous
    processing as done by this module, which is normally only a debug tool but not part of
    the true application SW. This is a debug switch, which can be set to 1 temporarily to
    make this module inactive. The API (and thus all dependent code) can still be compiled
    but no functionality and in particular no asynchronous interrupt code is compiled. (Of
    course, no characters are transmitted through the serial interface.)
      @remark Note, disabling the compilation has by intention made in a way that not all
    elements are switched off so that the compiler emits a lot of warning about unused
    stuff. This will help to avoid forgetting to reset the temporary configuration
    change. */ 
#define DEBUG_COMPILE_AS_EMPTY_STUB 0
...
#define IDX_LINFLEX_RX_IRQ  (79 + 20*IDX_LINFLEX_D)
    ...
    ihw_installINTCInterruptHandler( (int_externalInterruptHandler_t)
                                                {.simpleIsr = &dmaTransferCompleteInterrupt}
...
{
#if DEBUG_COMPILE_AS_EMPTY_STUB == 0
