...
/** The standard RTuinOS clock tic on the Arduino Mega 2560 board is 1/(16MHz/64/510) =
    51/25000s. We add 51 in each tick and have the next second when we reach 25000. This
    permits a precise implementation of the real time clock even with 16 Bit integer
    arithmetics. See #CLOCK_TICK_DENOMINATOR also. */
#define CLOCK_TICK_NUMERATOR     51

/** Denominator of ratio, which implements the clock's task rate. See #CLOCK_TICK_NUMERATOR
    for details. */
#define CLOCK_TICK_DENOMINATOR   (25000-(CLOCK_TICK_DENOMINATOR_TRIM_TERM))
...
/** Trim term for clock: By long term observation a correction has been figured out, which
    ...
    to the denominator #CLOCK_TICK_DENOMINATOR. It is device dependent. Starting point on a
    new hardware device should be 0. Positive values advance the clock. */
#define CLOCK_TICK_DENOMINATOR_TRIM_TERM 0
...
void clk_taskRTC()
{
    ...
    else
    {
        ...
        _noTaskTics += CLK_TASK_TIME_RTUINOS_STANDARD_TICKS*CLOCK_TICK_NUMERATOR;

    } /* if(A button has been touched to adjust the clock?) */    
    
    /* Carry ripple. */
    if(_noTaskTics >= CLOCK_TICK_DENOMINATOR)
    {
        _noTaskTics -= CLOCK_TICK_DENOMINATOR;
    ...
    if(doDisplay)
        iprintf("%02u:%02u:%02u" RTOS_EOL, clk_noHour, clk_noMin, clk_noSec);
