...
/** The API offered by the startup code differs slightly if package kernelBuilder is
    applied. You need to configure this explicitly. */
#define ADC_ENABLE_FOR_BUILD_WITH_KERNEL_BUILDER 1

/** This switch decides, whether the end-of-conversion interrupt is an ordinary interrupt
    or a kernel interrupt. An ordinary interrupt notifies the ADC result and needs to
    return to and continue with the interrupted execution context. A kernel interrupt can
    interact with an (external) kernel implementation and can then decide to return to and
    continue with another execution context. Normally used to implement a task switch to a
    task that evaluates the new ADC results.\n
      Note, kernel relevant interrupts can only be use together with the package
    kernelBuilder
    (https://github.com/PeterVranken/TRK-USB-MPC5643L/tree/master/LSM/kernelBuilder). */
#define ADC_ENABLE_INTERRUPT_AS_KERNEL_RELEVANT 1
...
#define ADC_USE_ADC_0_CHANNEL_15     1   /// Enable ADC_0, channel 15: Enable it for TSENS_0
...
#define ADC_USE_ADC_1_CHANNEL_15     1   /// Enable ADC_1, channel 15: Enable it for TSENS_1
...
/** The cycle time of the ADC conversions. The range is from about 10us to nearly 140ms, the
    ...
    The value of the macro is an unsigned long literal, unit is Microsecond.\n
      1024: We adopt the frequency of about 977 Hz from the original Arduino sample. */
#define ADC_T_CYCLE_IN_US   (1024ul)
...
/** The reference voltage of ADC_1. This value is used for proper configuration of ADC_1
    and for the absolute calibration of the ADC readings in Volt. See
    adc_getChannelVoltage().\n
      Note, the ADC has narrow ranges for valid reference voltages. These are basically
    values around 3.3 and 5 Volt. See MCU manual for more details, section 8.2, p. 139. */
...
#elif ADC_ENABLE_INTERRUPT_AS_KERNEL_RELEVANT != 0
# error Kernel interrupts are only available if option kernelBuilder is configured
#endif
