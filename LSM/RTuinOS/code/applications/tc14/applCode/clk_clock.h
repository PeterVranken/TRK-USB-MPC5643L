...
/** To be used to configure the regular task of the real time clock implementation: The
    task has to be called every #CLK_TASK_TIME_RTUINOS_STANDARD_TICKS RTuinOS standard
    system clock tics.
      @remark The chosen value doesn't strongly matter. The greater it is the lower is the
    overhead of the RTOS, but if it gets to large the update of the display might become
    visibly irregular. We choose a crude value to demonstrate the don't matter character. */
#define CLK_TASK_TIME_RTUINOS_STANDARD_TICKS 123
