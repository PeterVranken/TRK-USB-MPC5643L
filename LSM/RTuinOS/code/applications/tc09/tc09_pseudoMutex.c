...
static void taskEntryC0(uint32_t postedEventVec);
...
{
    /* No access synchronization to the flag is needed as a write of bool is atomic as
       such. */
    assert(_mutex);
    _mutex = false;
...
static void taskC0(unsigned int idxTask)
{
    ...
    while(true)  
    {
        ...
        iprintf("This is task %u", idxTask);
        ...
        iprintf("Now the resource Serial is released by task %u\r\n", idxTask);
