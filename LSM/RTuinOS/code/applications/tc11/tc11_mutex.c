...
/** Stack size of all the tasks in Byte. */
...
static _Alignas(uint64_t) uint8_t _stackT0_C0[STACK_SIZE]
 ...
 */ 
static void taskC0(unsigned int idxTask)
{
    unsigned int cnt = 0;
    ...
    while(true)  
    {
        ...
        iprintf("This is task "); iprintf("%u", idxTask);
        ...
        /* The ownership of the resource needs to be independent of the status of the
           ...
           delay returned. */
        iprintf(": "); iprintf("%u", ++cnt);
        iprintf(" loops. Thi");
        rtos_delay(TIME_IN_MS(12));
        iprintf("s line of console output is interrupted by seve");
        del_delayMicroseconds(31000/*us*/);
        iprintf("ral task de-activations. ");
        iprintf("Now the resource Serial is released by task ");
        iprintf("%u\r\n", idxTask);
 ...
 */
static void taskEntryC0(uint32_t initCondition)
{
...
#define TASK_TIME_T0_C1_MS  21
    ...
    do
    {
        ...
        /* The ownership of the resource needs to be independent of the status of the
           ...
           we have a high probability of running into a round robin task switch before
           delay returned. */
        iprintf("This is task T0_C1");
        iprintf(": "); iprintf("%u", ++cnt);
        iprintf(" loops. This line of conso");
        rtos_delay(TIME_IN_MS(7));
        iprintf("le output is interr");
        del_delayMicroseconds(3000/*us*/);
        iprintf("upted by several task de-activations");
        iprintf(". Now the resource is released again\r\n");
